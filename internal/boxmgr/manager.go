...
func (m *Manager) Start(ctx context.Context) error {
	...
	if cfg.Mode == "multi-port" || cfg.Mode == "hybrid" {
		...
		if err := cfg.NormalizeWithPortMap(portMap); err != nil {
			return fmt.Errorf("start normalize config with port map: %w", err)
		}
		used := make(map[uint16]bool)
		if cfg.Mode == "hybrid" {
			used[cfg.Listener.Port] = true
		}
		...
		cursor := cfg.MultiPort.BasePort
		for i := range cfg.Nodes {
			if cfg.Nodes[i].Port != 0 {
				continue
			}
			for used[cursor] || !isPortAvailable(cfg.MultiPort.Address, cursor) {
				cursor++
				if cursor > 65535 {
					return fmt.Errorf("start port completion failed: no available ports from %d", cfg.MultiPort.BasePort)
				}
			}
			cfg.Nodes[i].Port = cursor
			used[cursor] = true
			m.logger.Infof("Start: completed zero port for node[%d] name=%s -> %d", i, cfg.Nodes[i].Name, cursor)
			cursor++
		}
	}
...
func (m *Manager) ReloadWithPortMap(newCfg *config.Config, portMap map[string]uint16) error {
	...
	m.logger.Infof("ReloadWithPortMap: before normalize mode=%s base=%d nodes=%d", newCfg.Mode, newCfg.MultiPort.BasePort, len(newCfg.Nodes))
	for i, n := range newCfg.Nodes {
		if i >= 8 { break }
		m.logger.Infof("ReloadWithPortMap: before node[%d] name=%s port=%d uri=%s", i, n.Name, n.Port, n.URI)
	}
	...
	if newCfg.Mode == "multi-port" || newCfg.Mode == "hybrid" {
		used := make(map[uint16]bool)
		if newCfg.Mode == "hybrid" {
			used[newCfg.Listener.Port] = true
		}
		for _, n := range newCfg.Nodes {
			if n.Port > 0 {
				used[n.Port] = true
		...
		cursor := newCfg.MultiPort.BasePort
		for i := range newCfg.Nodes {
			if newCfg.Nodes[i].Port != 0 {
				continue
			}
			for used[cursor] || !isPortAvailable(newCfg.MultiPort.Address, cursor) {
				cursor++
				if cursor > 65535 {
					return fmt.Errorf("post-normalize port completion failed: no available ports from %d", newCfg.MultiPort.BasePort)
				}
			}
			newCfg.Nodes[i].Port = cursor
			used[cursor] = true
			m.logger.Infof("ReloadWithPortMap: completed zero port for node[%d] name=%s -> %d", i, newCfg.Nodes[i].Name, cursor)
			cursor++
		}
	}
	...
	for i, n := range newCfg.Nodes {
		if i >= 8 { break }
		m.logger.Infof("ReloadWithPortMap: after node[%d] name=%s port=%d uri=%s", i, n.Name, n.Port, n.URI)
	}
