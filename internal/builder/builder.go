...
func Build(cfg *config.Config) (option.Options, error) {
	log.Printf("🧪 Build entry: mode=%s base_port=%d nodes=%d", cfg.Mode, cfg.MultiPort.BasePort, len(cfg.Nodes))
	for i, n := range cfg.Nodes {
		if i >= 8 { break }
		log.Printf("🧪 Build entry node[%d]: name=%s port=%d uri=%s", i, n.Name, n.Port, n.URI)
	}
