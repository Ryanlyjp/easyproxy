...
`,
		...
		{
			Version:     3,
			Description: "add subscription grouping tables",
			Up: `
...
CREATE TABLE IF NOT EXISTS subscription_entries (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
    group_id   INTEGER NOT NULL REFERENCES subscription_groups(id) ON DELETE CASCADE,
    url        TEXT    NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_subscription_groups_sort_order ON subscription_groups(sort_order, id);
CREATE INDEX IF NOT EXISTS idx_subscription_entries_group_id ON subscription_entries(group_id);
CREATE INDEX IF NOT EXISTS idx_subscription_entries_sort_order ON subscription_entries(group_id, sort_order, id);
`,
		},
	}
}
