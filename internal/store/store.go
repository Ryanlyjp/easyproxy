...
type Store interface {
	...
	// ListSubscriptionGroups returns all subscription groups with nested entries.
	ListSubscriptionGroups(ctx context.Context) ([]SubscriptionGroup, error)

	// CreateSubscriptionGroup inserts a new subscription group.
	CreateSubscriptionGroup(ctx context.Context, group SubscriptionGroup) (*SubscriptionGroup, error)

	// CreateSubscriptionEntry inserts a new subscription entry into a group.
	CreateSubscriptionEntry(ctx context.Context, entry SubscriptionEntry) (*SubscriptionEntry, error)

	// ListEnabledSubscriptionEntryURLs returns URLs for enabled entries in enabled groups.
	ListEnabledSubscriptionEntryURLs(ctx context.Context) ([]string, error)
	// ListEnabledSubscriptionEntries returns enabled entries with group info preserved.
	ListEnabledSubscriptionEntries(ctx context.Context) ([]SubscriptionEntry, error)
	// FindSubscriptionGroupByName returns a grouped subscription by exact name.
	FindSubscriptionGroupByName(ctx context.Context, name string) (*SubscriptionGroup, error)
	// ListSubscriptionEntryAliases returns all non-empty aliases currently assigned to grouped subscription entries.
	ListSubscriptionEntryAliases(ctx context.Context) ([]string, error)

	// UpdateSubscriptionGroupEnabled updates a group's enabled state.
	UpdateSubscriptionGroupEnabled(ctx context.Context, id int64, enabled bool) error
	// DeleteSubscriptionGroup removes a group and all of its child entries.
	DeleteSubscriptionGroup(ctx context.Context, id int64) error

	// UpdateSubscriptionEntryEnabled updates an entry's enabled state.
	UpdateSubscriptionEntryEnabled(ctx context.Context, id int64, enabled bool) error
	DeleteSubscriptionEntry(ctx context.Context, id int64) error
...
// SubscriptionEntry represents a single subscription item inside a group.
...
// SubscriptionGroup represents a subscription group with nested entries.
