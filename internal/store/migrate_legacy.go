...
func MigrateLegacyData(ctx context.Context, s Store, configDir string, inlineNodes []LegacyNode, nodesFilePath string, subscriptions []string) error {
	...
	// 3. Import legacy flat subscriptions into grouped subscription tables
	if len(subscriptions) > 0 {
		groups, err := s.ListSubscriptionGroups(ctx)
		if err != nil {
			return fmt.Errorf("list subscription groups before legacy import: %w", err)
		}
		if len(groups) == 0 {
			group, err := s.CreateSubscriptionGroup(ctx, SubscriptionGroup{
				Name:      "Default",
				Enabled:   true,
				SortOrder: 1,
			})
			...
			for i, sub := range subscriptions {
				...
				if _, err := s.CreateSubscriptionEntry(ctx, SubscriptionEntry{
					GroupID:   group.ID,
					URL:       sub,
					...
					return fmt.Errorf("import legacy subscription %d: %w", i, err)
				}
				totalImported++
			}
			log.Printf("[store] imported %d legacy subscriptions into default group", totalImported)
		}
	}

	// 4. Import manual nodes from manual_nodes.txt
