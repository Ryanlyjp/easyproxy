...
type fetchedSubscriptionNode struct {
...
func (m *Manager) subscriptionEntries() []store.SubscriptionEntry {
	if m.store != nil {
		entries, err := m.store.ListEnabledSubscriptionEntries(m.ctx)
		if err == nil && len(entries) > 0 {
			return entries
		}
		if err != nil {
			m.logger.Warnf("failed to list enabled grouped subscriptions from store: %v", err)
		}
	}

	m.mu.RLock()
	defer m.mu.RUnlock()
	entries := make([]store.SubscriptionEntry, 0, len(m.baseCfg.Subscriptions))
	for i, subURL := range m.baseCfg.Subscriptions {
		entries = append(entries, store.SubscriptionEntry{ID: int64(i + 1), URL: subURL, Alias: fmt.Sprintf("subscription-%d", i+1), Enabled: true, SortOrder: i + 1})
	}
	return entries
}

func sanitizeSubscriptionSourceLabel(label string, index int) string {
	label = strings.TrimSpace(label)
	if label == "" {
		label = fmt.Sprintf("subscription-%d", index+1)
	}
...
func (m *Manager) fetchAllSubscriptions() ([]fetchedSubscriptionNode, error) {
	var allNodes []fetchedSubscriptionNode
	var lastErr error
	...
	for idx, entry := range m.subscriptionEntries() {
		label := sanitizeSubscriptionSourceLabel(entry.Alias, idx)
		source := store.NodeSourceSubscription + ":" + label
		nodes, err := m.fetchSubscription(entry.URL, timeout)
		if err != nil {
			m.logger.Warnf("failed to fetch %s: %v", entry.URL, err)
			lastErr = err
			continue
		}
		m.logger.Infof("fetched %d nodes from subscription %s", len(nodes), label)
		for _, node := range nodes {
			allNodes = append(allNodes, fetchedSubscriptionNode{Node: node, Source: source})
		}
	}
...
func (m *Manager) createNewConfig(subNodes []fetchedSubscriptionNode) *config.Config {
	...
	convertedSubNodes := make([]config.NodeConfig, 0, len(subNodes))
	for i := range subNodes {
		node := subNodes[i].Node
		node.Name = strings.TrimSpace(node.Name)
		node.URI = strings.TrimSpace(node.URI)
		node.Source = config.NodeSource(subNodes[i].Source)
		...
		convertedSubNodes = append(convertedSubNodes, node)
	}
	allNodes = append(allNodes, convertedSubNodes...)
