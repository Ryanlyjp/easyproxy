...
import type { SettingsData, SubscriptionStatus, SubscriptionGroup, SubscriptionEntry } from '../types'
import { fetchSettings, updateSettings, triggerReload, fetchSubscriptionStatus, refreshSubscription, createSubscriptionGroup, createSubscriptionEntry, toggleSubscriptionEntry, toggleSubscriptionGroup, deleteSubscriptionEntry, quickAddSubscription, deleteSubscriptionGroup } from '../api/client'
...
export default function SettingsPanel() {
  ...
  const [newEntryByGroup, setNewEntryByGroup] = useState<Record<number, string>>({})
  const groupedSubscriptions: SubscriptionGroup[] = settings.subscription_groups ?? []
  const hasGroupedSubscriptions = groupedSubscriptions.length > 0

  const syncGroupedSettings = (updater: (prev: SubscriptionGroup[]) => SubscriptionGroup[]) => {
    setSettings(prev => ({ ...prev, subscription_groups: updater(prev.subscription_groups ?? []) }))
    setSavedSettings(prev => ({ ...prev, subscription_groups: updater(prev.subscription_groups ?? []) }))
    setIsDirty(false)
  }

  const appendGroup = (group: SubscriptionGroup) => {
    syncGroupedSettings(groups => [...groups, group])
  }

  const appendEntry = (groupId: number, entry: SubscriptionEntry) => {
    syncGroupedSettings(groups => groups.map(group => {
      if (group.id !== groupId) return group
      const entries = [...group.entries, entry]
      const anyEnabled = entries.some(item => item.enabled)
      return { ...group, enabled: anyEnabled, entries }
    }))
  }
  ...
  const addSubscription = async () => {
    ...
    setError('')
    try {
      const { group, entry, message } = await quickAddSubscription(url) as { group: SubscriptionGroup; entry: SubscriptionEntry; message: string }
      syncGroupedSettings(groups => {
        const existing = groups.find(item => item.id === group.id)
        if (existing) {
          return groups.map(item => item.id === group.id ? { ...item, ...group, entries: [...(item.entries ?? []), entry] } : item)
        }
        return [...groups, { ...group, entries: [entry] }]
      })
      setNewSubUrl('')
      setSuccess(message || '订阅已添加到 Default')
    } catch (err) {
  ...
  const handleCreateGroup = async () => {
    ...
    setError('')
    try {
      const { group } = await createSubscriptionGroup(name) as { group: SubscriptionGroup }
      appendGroup({ ...group, entries: group.entries ?? [] })
      setNewGroupName('')
      setSuccess('分组已创建')
    } catch (err) {
  ...
  const handleCreateEntry = async (groupId: number) => {
    const url = (newEntryByGroup[groupId] ?? '').trim()
    if (!url) return
    setError('')
    try {
      const { entry } = await createSubscriptionEntry(groupId, url) as { entry: SubscriptionEntry }
      appendEntry(groupId, entry)
      setNewEntryByGroup(prev => ({ ...prev, [groupId]: '' }))
      setSuccess('订阅已加入分组')
    } catch (err) {
  ...
  const handleToggleEntry = async (id: number, enabled: boolean) => {
    setError('')
    try {
      await toggleSubscriptionEntry(id, enabled)
      syncGroupedSettings(groups => groups.map(group => {
        const entries = group.entries.map(entry => entry.id === id ? { ...entry, enabled } : entry)
        const anyEnabled = entries.some(entry => entry.enabled)
        return { ...group, enabled: anyEnabled, entries }
      }))
    } catch (err) {
      setError(err instanceof Error ? err.message : '切换订阅失败')
    }
  }

  const handleToggleGroup = async (id: number, enabled: boolean) => {
    setError('')
    try {
      await toggleSubscriptionGroup(id, enabled)
      syncGroupedSettings(groups => groups.map(group => group.id === id ? { ...group, enabled, entries: group.entries.map(entry => ({ ...entry, enabled })) } : group))
    } catch (err) {
      setError(err instanceof Error ? err.message : '切换分组失败')
    }
  }

  const handleDeleteEntry = async (groupId: number, id: number) => {
    if (!window.confirm('确认删除这个订阅吗？')) return
    setError('')
    try {
      const res = await deleteSubscriptionEntry(id)
      syncGroupedSettings(groups => groups.map(group => group.id === groupId ? { ...group, entries: group.entries.filter(entry => entry.id !== id) } : group))
  ...
  const handleDeleteGroup = async (groupId: number) => {
    if (!window.confirm('确认删除该分组吗？这会连带删除下面所有订阅链接。')) return
    setError('')
    try {
      const res = await deleteSubscriptionGroup(groupId)
      syncGroupedSettings(groups => groups.filter(group => group.id !== groupId))
      setSuccess(res.message || '分组已删除')
    } catch (err) {
    ...
    <div className="flex flex-col min-h-full animate-in fade-in duration-500">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-base-300/50 bg-base-100 p-6 lg:p-8 space-y-5 shadow-sm transition-shadow hover:shadow-md lg:col-span-2">
          ...
          {hasGroupedSubscriptions ? (
            <div className="space-y-4 mt-4">
              <div className="flex gap-2">
                <input
                  type="text"
                  className="input input-md flex-1 bg-base-200/50 focus:bg-base-100"
                  placeholder="新分组名称，如：高质量 / 备用 / 台湾"
                  value={newGroupName}
                  onChange={(e) => setNewGroupName(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && handleCreateGroup()}
                />
                <button className="btn btn-md btn-primary" onClick={handleCreateGroup} disabled={!newGroupName.trim()}>新增分组</button>
              </div>
              {groupedSubscriptions.map((group) => {
                const totalEntries = group.entries?.length ?? 0
                const enabledEntries = (group.entries ?? []).filter(entry => entry.enabled).length
                const allEnabled = totalEntries > 0 && enabledEntries === totalEntries
                const partiallyEnabled = enabledEntries > 0 && enabledEntries < totalEntries
                return (
                <div key={group.id} className="rounded-xl border border-base-300/60 bg-base-200/20 overflow-hidden">
                  <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-base-300/50 bg-base-100/60">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" className="checkbox checkbox-sm" checked={allEnabled || (group.enabled && !partiallyEnabled)} onChange={(e) => handleToggleGroup(group.id, e.target.checked)} />
                          <span className="font-semibold text-base-content">{group.name}</span>
                        </label>
                        <span className={`badge badge-sm ${group.enabled ? 'badge-success' : 'badge-ghost'}`}>{partiallyEnabled ? '半选' : group.enabled ? '已启用' : '已禁用'}</span>
                        <span className="badge badge-ghost badge-sm">{enabledEntries}/{totalEntries} 条</span>
                      </div>
                    </div>
                    <button
                      className="btn btn-sm btn-ghost text-base-content/50 hover:text-error hover:bg-error/10"
                  ...
                  <div className="p-3 space-y-2">
                    <div className="flex gap-2 pb-2">
                      <input
                        type="text"
                        className="input input-sm flex-1 bg-base-100/80"
                        placeholder="向当前分组添加订阅 URL"
                        value={newEntryByGroup[group.id] ?? ''}
                        onChange={(e) => setNewEntryByGroup(prev => ({ ...prev, [group.id]: e.target.value }))}
                        onKeyDown={(e) => e.key === 'Enter' && handleCreateEntry(group.id)}
                      />
                      <button className="btn btn-sm btn-primary" onClick={() => handleCreateEntry(group.id)} disabled={!(newEntryByGroup[group.id] ?? '').trim()}>添加订阅</button>
                    </div>
                    {(group.entries?.length ?? 0) > 0 ? group.entries.map((entry) => (
                      <div key={entry.id} className="flex items-start gap-3 p-3 rounded-lg border border-base-200 bg-base-100/70">
                        <input type="checkbox" className="checkbox checkbox-sm mt-0.5" checked={entry.enabled} onChange={(e) => handleToggleEntry(entry.id, e.target.checked)} />
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            <span className="font-medium text-sm text-base-content">{entry.alias?.trim() || '未命名订阅'}</span>
                            <span className={`badge badge-xs ${entry.enabled ? 'badge-success' : 'badge-ghost'}`}>{entry.enabled ? '勾选' : '未勾选'}</span>
                          </div>
                          <code className="text-xs font-mono text-base-content/75 break-all">{entry.url}</code>
                        </div>
                        <button
                          className="btn btn-sm btn-square btn-ghost text-base-content/40 hover:text-error hover:bg-error/10 shrink-0"
                          onClick={() => handleDeleteEntry(group.id, entry.id)}
                          title="删除订阅"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <div className="text-sm text-base-content/50 px-2 py-3">该分组下暂无订阅</div>
                    )}
                  </div>
                </div>
              )})}
          ...
          <p className="text-xs text-base-content/40 text-center mt-4">⚠️ 当前已优先展示后端返回的分组订阅数据；完整分组编辑交互正在接入，现阶段原扁平订阅编辑仍作为兼容入口保留。</p>
        </div>
      </div>
